...
This component allows us to override the basic HTML head.

- **`<Head>` in `SSR` and `SSG` pages** : The component `Head` will be rendered by server, so browser will receve an HTML file which includes meta tags that we defined.

- **`<Head>` in CSR pages** : this component will be called during **client-side-rendering** so it overrides the HTML head tag which is generated by `_document.jsx` on server.

**NOTE** : If we use two `<Head>` components in the same page the second one will override the first one.

```jsx
<Head>
  <title>Home page</title>
  <meta name="describtion" content="This is a meta tag to describe this page" />
</Head>
```

Defining `<Head>` in `_app.jsx` as global for all pages and also overriding it.
...
export default function App({ Component, pageProps }) {
  return (
    <>
      <Head>
        <link rel="icon" type="image/x-icon" href="/images/global-fav-icon.png" />
      </Head>
...
// Overrider fav icon ======= >
export default function ProductInfo({ Component, pageProps }) {
  return (
    <>
      <Head>
        <link rel="icon" type="image/x-icon" href="/images/product-fav-icon.png" />
      </Head>
      <main style={{ padding: "20px" }}>
        <Component {...pageProps} />
      </main>
...
An example of using `Head` in `SSR` :
